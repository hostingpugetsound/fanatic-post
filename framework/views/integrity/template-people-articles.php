...
?>
	  ...
	  <ul id="navlist" style=" margin-bottom:20px">
		<li><a href="<?php echo get_site_url();?>/people/<?php echo $post->post_name; ?>">The Arena <?php comments_number( '', '<span class="circleNum">1</span>', '<span class="circleNum">%</span>' ); ?></a></li>
		<li class="<?php if(strstr($_SERVER['REQUEST_URI'], 'article')) { echo "active"; } ?>"><a href="<?php echo get_site_url();?>/article/?p=<?php echo $post->post_name; ?>">Articles <span class="circleNum"><?= $articlesNum; ?></span></a></li>
	</ul>
  ...
  <div class="x-container max width " style="width:100%;">
    <div >
		...
		<div style="float:right;">
                    <span class="saySomething">Have something to say about <?php the_title(); ?>?</span><br />
            ...
            ?>
            <a href="<?php echo $agreement_url; ?>" class="x-btn  x-btn-round x-btn-x-small"><?php echo ($current_user->ID && get_option('articleprepayment' . $current_user->ID))? 'Post Your Article (1)': 'Be a Sportswriter $5'; ?></a>  &nbsp;&nbsp;&nbsp; <?php echo do_shortcode('[tooltip title="Be a Sportswriter" text="Think you have what it takes to be a trusted voice in sports? Look no further, we are looking for fanatics just like you. Post an article here, on this page, and share in the revenue that your individual article page produces."]Learn More[/tooltip]'); ?>
		</div>
		<div style="clear:both;">&nbsp;</div>
...
<?php /*	 <a href="<?php the_permalink() ?>" rel="bookmark" title="Permanent Link to <?php the_title_attribute(); ?>" class="x-btn  x-btn-round x-btn-x-small">Read Article</a><span style="margin-left:10px;"> (<?php comments_number(); ?>)</span> */ ?>
         ...
         <?php
           ...
           $current_url = $pp_app_base_url . '/article/?p=' . $_GET['p'];
       ...
       ?>
         ...
         <?php if(!$post->promoted && $current_user->ID == $post->post_author && $promoted_count < 3): ?>
            <form action="<?php echo PAYPAL_URL;?>" method="post" style="position: absolute; bottom: -50px;">
                ...
                <input type="submit" class="promoteBtn" value="Promote this Article $5" />

                &nbsp;&nbsp;&nbsp; <?php echo do_shortcode('[tooltip title="Promote Your Article" text="By promoting your article, this article will appear above all other articles on this page, except other promoted article posts on this page, of which there can be a maximum total of 3 promoted article posts at any given time. We will also place a link to your promoted article on this teams Arena page. This promotion will last for 3 days (72 Hours), at which time, your article will be released back into the general population of articles specific to this team."]Learn More[/tooltip]'); ?>
            </form>
         <?php endif; ?>
