...
if ( ! function_exists( 'x_integrity_comment' ) ) :
  function x_integrity_comment( $comment, $args, $depth ) {
    ...
    switch ( $comment->comment_type ) :
      ...
      case 'trackback' : // 1
    ?>
    <li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>" class="fan">
    ...
    ?>
    <li id="li-comment-<?php comment_ID(); ?>" <?php comment_class(); ?>  class="fan">
      <?php
      printf( '<div class="x-comment-img">%1$s %2$s</div>',
        '<span class="avatar-wrap cf' . $avatar_variation . '">' . get_avatar( $comment, 120 ) . '</span>',
        ( $comment->user_id === $post->post_author ) ? '<span class="bypostauthor">' . __( 'Fan', '__x__' ) . '</span>' : ''
      );
      ?>
...
if ( ! function_exists( 'x_integrity_comment_2' ) ) :
